...
class AbonoController extends Controller
{
    ...
    */
     ...
     * 
     *
     * @Route("/abono/data/listado/vacuna", name="admin_abonos_paciente_data_vacuna")
     */
    public function dataAbonoVacunaAction(Request $request)
    {
        ...
        $expedientesTotal = $em->getRepository('DGPlusbelleBundle:Abono')->findBy(array('ventaVacuna'=>$id));
...
//            $dql = "SELECT exp.numero as expediente, pac.id as id,CONCAT(per.nombres, per.apellidos) as nombres, '<a ><i style=\"cursor:pointer;\"  class=\"infoPaciente fa fa-info-circle\"></i></a>' as link FROM DGPlusbelleBundle:Incapacidad inc "
            $dql = "SELECT DATE_FORMAT(ab.fechaAbono,'%d-%m-%Y %H:%i') as fechaAbono, CONCAT(per.nombres,' ', per.apellidos) as empleado, concat('<div class=\"pull-right\">',ab.monto,'</div>') as monto FROM DGPlusbelleBundle:Abono ab "
                    . "JOIN ab.empleado emp "
                    . "JOIN emp.persona per "
                    //. "JOIN ab.sucursal suc "
                    . "WHERE ab.ventaVacuna=:id "
